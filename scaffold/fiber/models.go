...
func (fp FiberProject) createModelFiles() error {
	userModelFileBody := strings.ReplaceAll(userModelFileTemplate, "${moduleName}", fp.ModuleName)
	...
	if err := os.WriteFile(fmt.Sprintf("%s/models/user.model.go", fp.Directory), []byte(userModelFileBody), 0644); err != nil {
		return err
	}
